...
| 16 | **Any issuer instance will mint any published credential type**, signing with that credential's author key over its OWN entity's data | `ADVERTISE_OWN_CREDENTIALS_ONLY` (finding 14) filters issuer METADATA. It does not restrict `POST /oid4vc/credential`, which accepts any published `credential_configuration_id`. Found on 1 September 2026 while writing Education's issuance tests: asking the **school** instance for the **college** configuration returned HTTP 200 and a credential whose `iss` is the College's DID — credentials-service signs with the schema `author` key — carrying the learner's **school** percentage (78.5, where the real college figure is 71.2). The Agriculture equivalent of that test passes, which is why this went unnoticed for an iteration: the Land schema requires claims a FarmerRecord lookup cannot supply, so cross-issuance there fails on **validation**, not on authorization. Education's three records share their claim shape, so nothing stops it | **Contained today, and only by the `vct`.** oid4vc-service normalises a relative `vct` against the MINTING instance's `PUBLIC_URL`, so the cross-minted credential's type is `<host>/school/vct/college-record-credential` — not the `<host>/college/vct/...` that both portals pin in their DCQL query. The presentation is therefore refused, and `tests/e2e/flow3-education-issuance.test.mjs` asserts BOTH halves: that the mint succeeds, and that neither portal accepts the result. No verifier change was needed. **CLOSED on Anand's instruction, 2 September 2026.** It was raised rather than fixed because a trust boundary resting on how a URL is constructed is not an authorization check, and changing that is his call; he asked for the restriction to apply to the endpoint itself. `buildSelfServiceSession` now resolves against the same own-authored list `issuerMetadata()` is built from, so advertising and issuing cannot diverge again — that divergence was the whole defect. A request for another issuer's type is refused **by name**: *"'College Record Credential' is not issued by this issuer. It is authored by did:web:…; request it from that issuer instead."* Including a `vct`-only request, which is the shape a wallet on the authorization_code path actually sends and which would otherwise have been refused with the useless *"could not determine the credential type"*. Matching there is on the type **slug**, because every instance normalises a relative `vct` against its own `PUBLIC_URL` and comparing whole `vct`s across instances is always false. Same flag and same `author` comparison as finding 14, so the same removal path. Covered by `own-credentials-issuance.spec.ts` (7 tests, positive and negative), by `tests/e2e/flow3-education-issuance.test.mjs` → *an institution issues only its own credential type* (4 tests, both request shapes, both directions, and a no-leak assertion), and by a `verify.sh` check on the running containers. The two tests that asserted the DEFECT are gone, and the comment left in their place says what they were and where their containment assertion went |
| 17 | A holder could **disclose more than the request asked for** and be told the exchange succeeded | `DcqlService.evaluate()` built its result from the requested paths alone: it checked that every requested claim was present and never that no unrequested one was. The surplus was therefore dropped rather than refused — the relying party never saw it and no decision could turn on it, but the values had left the wallet and reached this service, so *"the verifier never receives it"* was true of the relying party and not of the protocol boundary. Recorded as an Education deviation on 1 September 2026 and sent back by Anand: §8 lists over-disclosure among the things to **reject** | fixed in the fork. The matcher compares the disclosed claim names against the query and refuses the credential, naming the surplus claims and never their values — a diagnostic that echoed the value would disclose what it had just refused to accept. On the FIRST path segment, since a nested claim is disclosed as its top-level object. Only for selective-disclosure formats, and only when the query named claims: a query with no `claims` asks for the whole credential, so nothing can exceed it. `REJECT_UNREQUESTED_DISCLOSURES=false` restores the old behaviour; unlike finding 14's flag this defaults **ON**, because accepting undisclosed data is not a safe default. **A second defect inside the first:** the wiring was inert on the multi-credential keyed `vp_token` path — the path every SD-JWT presentation in this deployment takes — because that branch built its `presented` entry without `disclosedNames`. Eight unit tests passed throughout, all of them handing the matcher a fixture written by hand; the end-to-end test against a live stack is what caught it. Three tests now drive the matcher from a real SD-JWT through the actual `extractCredentials()`, with no fixture in between. **Removal path:** delete the flag and the comparison if upstream rejects unrequested disclosures itself |
| 18 | The wallet warns **"No information was provided on the purpose of the data request. Be cautious"** on every review screen | accurate, and shipped that way through two iterations — narrated aloud in both films rather than hidden. `client_metadata` was the obvious place to look and is the wrong one: it describes the client, the wallet SDK does not read a purpose from it, and a request carrying it there still warns. The field the wallet actually consults is OpenID4VP 1.0 §6.1's `credential_sets[].purpose` — `packages/sdk/src/format/dcqlRequest.ts` takes the first set whose `purpose` is a string | fixed **repo-side**, not in the fork: `services/verifier/src/core/dcql.mjs` emits one `credential_sets` entry, `required: true`, listing every credential id and carrying the purpose the use case already publishes at `/policy`. One set rather than one per credential, because the wallet takes the first purpose it finds and per-credential sets would leave the others unexplained; every id in one option rather than split across options, because splitting them would make a three-credential request satisfiable by fewer — a change to what was asked for rather than to how it is explained. A present-but-blank purpose is rejected outright: swallowing it would ship the warning screen while looking fixed. Age and Agriculture declare no purpose and their requests stay byte-identical, since both accepted films narrate the notice out loud and changing them is not this iteration's call. Covered by six tests in `tests/unit/dcql.test.mjs`, three in `tests/unit/wallet-trust.test.mjs` asserting the verifier fills the same field the vendored SDK reads, and a `verify.sh` check that decodes the **signed request object** and compares its purpose to the published policy. **Still to confirm on hardware:** that the consent screen renders it. No device was attached when this landed |

| 19 | **[WALLET BACKLOG]** A same-device **deep link crashes the wallet** for some verifiers and not others, with *"Something went wrong"* on screen and `CredoError: Could not decode data from base64 string` in the log | found on 2 September 2026 while confirming finding 18 on the device. A deep link always forces re-authentication first, so `+native-intent.tsx` stores the onward path as a query parameter — `/authenticate?redirectAfterUnlock=<base64>`. It **encodes** with `TypedArrayEncoder.toBase64Url()` (`+native-intent.tsx:93`, `(app)/_layout.tsx:46`) and **decodes** with `TypedArrayEncoder.fromBase64()` (`authenticate.tsx:94`). Two mismatches in one line: base64url is unpadded where `fromBase64` is strict, and base64url's alphabet is `-`/`_` where `fromBase64` expects `+`/`/`. So the route survives only when the payload length happens to be a multiple of 4 — measured, not guessed: the Master's link is 392 characters (`% 4 == 0`, worked on every attempt) and the employer link is 390 (`% 4 == 2`, failed on every attempt). Both lengths are fixed per portal, since the verifier path and session id are fixed-length, which is why this looks deterministic per verifier rather than intermittent | **not fixed, and not blocking.** The Education charter's journey is a cross-device QR — `DEMO.md`'s "verifier websites on a separate screen" — and the QR route does not go through this code at all, so nothing the film shows is affected and the employer consent screen was confirmed that way. Part 9's installed mobile verifier app is `education-masters`, the length that works, which is why two iterations of same-device deep links never hit it. **The fix is one word:** `fromBase64Url` in place of `fromBase64` at `authenticate.tsx:94`, which matches what both encode sites already produce. Deliberately not applied yet: it only takes effect in a rebuilt APK, and every recorded Age, Agriculture and Education artefact was produced with the installed `1.20.3` build, so rebuilding means re-verifying the issuance and presentation trust screens on the device before the evidence can be trusted again. That is Kartheek's call, not a change to make quietly mid-review. **Removal path:** drop it if upstream aligns the two calls |
